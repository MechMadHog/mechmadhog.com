---
layout: default
title: Movies & TV
permalink: /movies-tv/
---

<section class="page-section movies-page">

  <header class="section-header">
    <p>
      Films and television have always been a huge source of inspiration for me:
      the performances, the atmosphere, the soundtrack choices, the dialogue,
      and the strange little details that stay lodged in your head years later.
    </p>

    <p>
      I keep track of what I watch on
      <a href="https://letterboxd.com/mechmadhog/" target="_blank" rel="noopener noreferrer">
        Letterboxd
      </a>.
    </p>
  </header>

  <section class="content-card">
  <h2>⭐ Favourite Films</h2>
    <p>
      My favourite films are not necessarily the ones I think are technically
      the greatest. They are the films whose characters, worlds and ideas contain
      little pieces of my own personality.
    </p>
    <p>
      Sometimes it is a character I relate to or a particular characteristic I
      recognise in myself. Sometimes it is how relatable or self-aware the story
      feels, or simply how cool the characters and the world around them are.
    </p>
    <p>
      They are funny, strange, exciting and occasionally sad—but sad in a way
      that is still enjoyable to return to.
    </p>
    <ul class="film-list">
      <li><em>Pump Up the Volume</em></li>
      <li><em>The Accountant</em></li>
      <li><em>Big Trouble in Little China</em></li>
      <li><em>Scream</em></li>
    </ul>
  </section>

  <section class="content-card">
    <h2>🎥 Letterboxd</h2>

    <p>
      I log films, rate what I watch, keep track of favourites, and occasionally
      discover something completely unexpected.
    </p>

    <p>
      You can follow along here:
      <a href="https://letterboxd.com/mechmadhog/" target="_blank" rel="noopener noreferrer">
        letterboxd.com/mechmadhog
      </a>
    </p>
  </section>

  <section class="content-card">
    <h2>💬 Favourite Quotes</h2>

    <p>
      Some films stay with you because of a performance. Others because of a single line that refuses to leave your head.
    </p>

    <p>
      I collect some of my favourite lines from film and television on my <a href="{{ '/quotes/' | relative_url }}">Quotes page</a>.
    </p>
  </section>

</section>